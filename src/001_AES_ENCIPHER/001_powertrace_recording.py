import encipher_api, scope_api, os, pickle
"""
...
def main(iterations = 10000, channel = 1):
	cphi = encipher_api.EncipherAPI("COM3")
	...
	for s in range(10):
		...
		for i in range(s*10, int(iterations / 10 + s*10)):
			...
			ciphertext = cphi.get_state()

			all_data.append((state, trace, ciphertext))
		
		filename = f"resultsf{s}.pkl"

		print(f"Saving iteration {iterations / 10 * (s + 1)}")
...
if __name__ == '__main__':
	main(iterations = 10000, channel = 1)
